...
### 4. 外部生产管理系统集成 (最新更新)
- **连接机制**：支持通过公司名、用户名、密码登录外部系统 (`www.wanpuxx.com`)，并持久化 Session Cookie。
- **数据同步**：
    - 自动获取颜色、尺码、原料字典数据。
    - 优化了 API 请求，支持一次性获取 1000 条以上数据。
    - 支持在选择弹窗中直接新增字典项并同步至外部系统。
- **UI/UX 优化 (CreateProductModal)**：
    - **弹窗选择器**：将颜色、尺码、原料改为大弹窗选择，支持搜索和批量确认。
    - **紧凑布局**：优化间距与字体，确保核心信息在一屏内展示。
    - **原料规格显示**：原料选择列表中突出显示颜色，并保留加粗深色的规格信息 (如 2/16)，默认单位统一为“克”。
    - **高对比度**：全界面文字加深，提升易读性。
